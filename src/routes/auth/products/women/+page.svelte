<script>
	...
	export let data;
	let { session, supabase, userNow, womenAll, itemCount } = data;
	$: ({ session, supabase, userNow, womenAll, itemCount } = data);
...
>
	<!-- {#each womenAll as category}{/each} -->
</section>

<pre>{JSON.stringify(womenAll, null, 2)}</pre>
